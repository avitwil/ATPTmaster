...
# --- expose the `atpt` command ---------------------------------------------
# Always install a lightweight launcher into ~/.local/bin so `atpt` works from
# anywhere (no pipx required). It pins ATPT_HOME/PYTHONPATH to this checkout so
# modules, var/ and the DB resolve here regardless of the current directory.
BIN_DIR="$HOME/.local/bin"; mkdir -p "$BIN_DIR"
cat > "$BIN_DIR/atpt" <<EOF
#!/usr/bin/env bash
# ATPTmaster launcher (generated by install.sh)
export ATPT_HOME="$here"
...
if [ "$DO_PIPX" = 1 ]; then
  command -v pipx >/dev/null && { pipx install -e "$here" && echo "[*] also installed via pipx"; } \
    || echo "[!] pipx not found — skip (the launcher above already provides 'atpt')"
fi
